<template>
  <div class="flex gap-6 overflow-x-auto py-4">
    <div
      class="flex min-w-0 w-[var(--ui-board-column-width-min)] min-h-[var(--ui-board-column-min-height)] flex-col rounded-xl bg-danger-50 dark:bg-danger-900/20"
      :class="{ 'border-2 border-dashed border-danger-500': dragOverColumn === 'unassigned' }"
      @dragover.prevent
      @dragenter="dragOverColumn = 'unassigned'"
      @dragleave="dragOverColumn = null"
      @drop="handleDropUnassigned"
    >
      <div class="flex items-center justify-between border-b-2 border-gray-200 p-4 dark:border-dark-700">
        <span class="font-bold">待分派任务</span><span class="inline-flex items-center justify-center px-2 py-0.5 text-xs font-medium rounded-full bg-warning-100 text-warning-700 dark:bg-warning-900/30 dark:text-warning-400">{ unassignedTasks.length }</span>
      </div>
      <div class="flex-1 overflow-y-auto p-4">
        <div
          v-for="task in unassignedTasks"
          :key="task.id"
          class="mb-3 cursor-move rounded-lg bg-white p-3 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
        ...
        >
          <div class="mb-2 flex items-start justify-between gap-2">
            <div class="flex-1 truncate font-bold text-sm">
              ...
              :status="task.work_order__priority"
              category="priority"
              size="small"
            />
          </div>
    ...
    <div
      ...
      :key="operator.id"
      class="flex min-w-0 w-[var(--ui-board-column-width-min)] min-h-[var(--ui-board-column-min-height)] flex-col rounded-xl bg-gray-50 dark:bg-dark-700"
      :class="{ 'border-2 border-dashed border-primary-500': dragOverOperator === operator.id }"
      @drop="handleDropOperator(operator)"
      @dragover.prevent
      @dragenter="dragOverOperator = operator.id"
      @dragleave="dragOverOperator = null"
    >
      <div class="flex items-center justify-between border-b-2 border-gray-200 p-4 dark:border-dark-600">
        <span class="font-bold">{{ operator.name }}</span><span class="inline-flex items-center justify-center px-2 py-0.5 text-xs font-medium rounded-full bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-400">{ getOperatorTaskCount(operator.id) }</span>
      </div>
      <div class="flex-1 overflow-y-auto p-4">
        <div
          v-for="task in (operatorTasks as any)[operator.id] || []"
          :key="task.id"
          class="mb-3 cursor-move rounded-lg bg-white p-3 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
          :class="getPriorityClass(task)"
          draggable="true"
        ...
        <EmptyState
          v-if="!(operatorTasks as any)[operator.id]?.length"
          description="暂无任务"
        />
      </div>
    </div>
