    ...
    <div class="space-y-4">
      ...
      <div>
        ...
        >
          ...
          <span
            v-if="(task.quantity_completed || 0) < task.production_quantity"
            class="font-medium text-warning-600"
          >（当前完成数量小于计划数量，将强制标记为已完成）</span>
        </div>
      </div>
      <div>
        <label class="input-label mb-1.5 block">完成理由</label><TextArea
      ...
      <div>
        <label class="input-label mb-1.5 block">选择图稿</label>
        <Select
          v-model="artworkSelectedValue"
          :options="artworkOptions"
          placeholder="请选择图稿"
          searchable
          :disabled="loadingArtworks"
          @click="loadArtworkList"
        />
        <div
          v-if="formData.artwork_ids.length > 0"
          class="mt-2 flex flex-wrap gap-1"
        >
          <Tag
            v-for="id in formData.artwork_ids"
            :key="id"
            type="info"
            size="small"
            class="cursor-pointer"
            @click="removeArtwork(id)"
          >
            {{ getArtworkLabel(id) }} <span class="ml-1 text-gray-400">&times;</span>
          </Tag>
        </div>
        <div class="text-xs text-gray-400 mt-1">
          选中的图稿将自动关联到施工单
      ...
      <div>
        <label class="input-label mb-1.5 block">选择刀模</label>
        <Select
          v-model="dieSelectedValue"
          :options="dieOptions"
          placeholder="请选择刀模"
          searchable
        ...
        />
        <div
          v-if="formData.die_ids.length > 0"
          class="mt-2 flex flex-wrap gap-1"
        >
          <Tag
            v-for="id in formData.die_ids"
            :key="id"
            type="info"
            size="small"
            class="cursor-pointer"
            @click="removeDie(id)"
          >
            {{ getDieLabel(id) }} <span class="ml-1 text-gray-400">&times;</span>
          </Tag>
        </div>
        <div class="text-xs text-gray-400 mt-1">
          选中的刀模将自动关联到施工单
      ...
      <div>
        <label class="input-label mb-1.5 block">任务备注</label><TextArea
