  ...
  <div class="mt-6">
    ...
    >
      ...
      <template #cell-task_type="{ row }">
        <StatusTag
          :status="row.task_type"
          category="taskType"
          :label="row.task_type_display"
          size="small"
        />
      </template>
      ...
      <template #cell-operator="{ row }">
        {{ row.assigned_operator_name || '未分配' }}
      </template>
      ...
      <template #cell-deadline="{ row }">
        <span :class="isOverdue(row) ? 'font-bold text-danger-600 dark:text-danger-400' : ''">{{ formatDate(getTaskDeadline(row)) }}</span>
      </template>
