<template>
  <div class="flex h-full flex-col rounded-xl bg-gray-100 overflow-hidden dark:bg-dark-800">
    <div
      class="flex items-center justify-between rounded-t-xl p-4 font-bold text-white"
      :class="headerGradient"
    >
      <span class="text-base">{{ title }}</span>
      <span class="rounded-lg bg-white/30 px-2 py-1 text-sm font-bold">{{ taskCount }}</span>
    </div>
    <div class="relative flex-1 overflow-y-auto p-4 max-h-[min(640px,calc(100vh-350px))]">
