...
interface Props {
  modelValue: string | number | boolean | null | undefined | (string | number | boolean)[]
...
// Chinese placeholders (no i18n in this project)
...
const selectedOption = computed(() => {
  if (props.multiple && Array.isArray(props.modelValue)) {
    return props.modelValue.map(v => props.options.find((opt) => getOptionValue(opt) === v)).filter(Boolean) || []
  }
...
const selectedLabel = computed(() => {
  ...
  if (props.multiple && Array.isArray(props.modelValue)) {
    ...
    if (props.modelValue.length === 1) {
      ...
      return opt ? getOptionLabel(opt) : String(val)
    }
...
const filteredOptions = computed(() => {
  ...
  if (isSearchable.value && searchQuery.value) {
    ...
    if (props.creatable && searchQuery.value.trim()) {
      ...
      const prefix = props.creatablePrefix || '搜索'
...
const selectOption = (option: any) => {
  ...
  if (props.multiple) {
    // Multiple selection: toggle value in/out of array
    const currentValues = Array.isArray(props.modelValue) ? [...props.modelValue] : []
    const existingIndex = currentValues.findIndex(v => v === value)
    ...
    emit('change', currentValues, option)
