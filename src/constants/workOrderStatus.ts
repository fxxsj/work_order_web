 ...
 * @returns {string} 状态标签
 */
export function getWorkOrderStatusLabel(status: any) {
  const choice = WorkOrderStatusChoices.find((item: any) => item.value === status)
 ...
 * @returns {string} 颜色值
 */
export function getWorkOrderStatusColor(status: any) {
  const choice = WorkOrderStatusChoices.find((item: any) => item.value === status)
