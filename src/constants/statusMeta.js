...
export function getStatusMeta(category, value, fallback = {}) {
  const categoryMap = StatusMetaMaps[category] || {}
  const meta = categoryMap[value] || {}
  return {
    text: fallback.text || meta.text || value || '-',
    type: fallback.type || meta.type || 'info',
    icon: fallback.icon || meta.icon || ''
  }
}

export function getStatusType(category, value, fallbackType = 'info') {
  return getStatusMeta(category, value, { type: fallbackType }).type
}

export function getStatusText(category, value, fallbackText = '') {
