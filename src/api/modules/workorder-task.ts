...
class WorkOrderTaskAPI extends BaseAPI {
  ...
  // 分派任务（调整分派）
  assign(id: number | string, data?: unknown) {
  ...
  // 获取部门操作员列表
  getDepartmentOperators(departmentId: number | string) {
    return this.request({
  ...
  // 操作员认领任务
  claimTask(id: number | string, data: Record<string, unknown>) {
  ...
  // 获取当前用户可认领的任务列表
  getClaimableTasks(params?: Record<string, unknown>) {
    return this.request({
  ...
  getOperatorCenterData(params?: Record<string, unknown>) {
    return this.request({
  ...
  // 分割任务
  split(id: number | string, data: Record<string, unknown>) {
    return this.customAction(`${this.baseUrl}${id}/split/`, 'post', data)
  }

  // 获取协作统计数据
  getCollaborationStats(params?: Record<string, unknown>) {
    return this.request({
  ...
  // 获取部门工作负载统计（主管看板）
  getDepartmentWorkload(params?: Record<string, unknown>) {
    return this.request({
  ...
  // 取消任务
  cancel(id: number | string, data: Record<string, unknown>) {
    return this.customAction(`${this.baseUrl}${id}/cancel/`, 'post', data)
  }

  // 批量更新数量
  batchUpdateQuantity(data: Record<string, unknown>) {
    return this.customAction(`${this.baseUrl}batch_update_quantity/`, 'post', data)
  }

  // 获取工序负责部门
  getProcessDepartments(processId: number | string) {
    return this.request({
   ...
   */
  exportExcel(options: Record<string, unknown> = {}) {
    return this.request({
  ...
  async bulkDelete(taskIds: (number | string)[]) {
    ...
    return {
      data: {
        ...
        failed_tasks: failed.map((item: any) => ({ id: item.id }))
      }
    }
  }

  // 带错误处理的任务认领
  async claimTaskWithErrorHandling(id: number | string, data: Record<string, unknown>) {
    try {
  ...
  // 带错误处理的任务分配
  async assignToOperatorWithErrorHandling(id: number | string, data: Record<string, unknown>) {
    try {
   ...
   * @returns {Promise}
   */
  batchAssign(data: Record<string, unknown>) {
    return this.request({
   ...
   * @returns {Promise}
   */
  batchComplete(data: Record<string, unknown>) {
    return this.request({
   ...
   * @returns {Promise}
   */
  batchCancel(data: Record<string, unknown>) {
    return this.request({
   ...
   * @returns {Promise}
   */
  batchDeleteTasks(data: Record<string, unknown>) {
    return this.request({
