<template>
  ...
  <embossing-plate-form-dialog v-model="dialogVisible" :dialog-type="dialogType" :embossing-plate="currentEmbossingPlate" :loading="formLoading" :product-list="productList" @confirm="handleFormConfirm" />
</template>
...
const handleDelete = async (row: any) => {
  try { const confirmed = await ErrorHandler.confirm(`确定要删除压凸版"${row.name}"吗？此操作不可撤销。`); if (!confirmed) return; await crud.remove(row.id, '删除成功') } catch (error: any) { ErrorHandler.showMessage(error, '删除失败') }
}

const handleConfirmPlate = async (row: any) => {
  try { const confirmed = await ErrorHandler.confirm(`确定要确认压凸版"${row.name}"吗？确认后关键字段将不可修改。`); if (!confirmed) return; await embossingPlateAPI.confirm(row.id); ElMessage.success('确认成功'); await loadData() } catch (error: any) { ErrorHandler.showMessage(error, '确认失败') }
}
