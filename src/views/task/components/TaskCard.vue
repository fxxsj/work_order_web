<template>
  <div class="card card-hover mb-3 cursor-pointer" :class="{ 'border-l-4 border-l-danger-500': isOverdue }" @click="emit('click', task)">
    <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
      <div class="flex flex-wrap items-center gap-2">
        <span class="text-xs font-bold text-gray-500 dark:text-dark-400">{{ orderNumber }}</span>
        <StatusTag v-if="priority" :status="priority" category="priority" :label="priorityDisplay" size="small" />
      </div>
      <StatusTag :status="task.task_type" category="taskType" :label="taskTypeDisplay" size="small" />
    </div>
    <div class="mb-3">
      <div class="mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ task.work_content }}</div>
      <div class="mb-2 flex justify-between text-xs text-gray-500 dark:text-dark-400">
        <span><Icon name="user" class="mr-1 inline h-3 w-3" />{{ operatorName }}</span>
        <span><Icon name="chartBar" class="mr-1 inline h-3 w-3" />{{ progressText }}</span>
      </div>
      <div v-if="deadlineText" class="mb-2 flex items-center gap-1 rounded-lg px-2 py-1 text-xs" :class="deadlineClass"><Icon name="clock" class="h-3 w-3" />{{ deadlineText }}</div>
      <div v-if="hasDefective" class="mb-2 flex items-center gap-1 text-xs text-warning-600 dark:text-warning-400"><Icon name="exclamationTriangle" class="h-3 w-3" />不良品：{{ task.quantity_defective }}</div>
      <ProgressBar :percentage="progressPercentage" :color="progressColor" :show-text="false" :stroke-width="6" />
    </div>
    <div class="flex gap-2 border-t border-gray-200 pt-2 dark:border-dark-700">
      <button v-if="canUpdate" class="btn btn-primary btn-sm flex-1" @click.stop="emit('update', task)">更新</button>
      <button v-if="canAssign" class="btn btn-warning btn-sm flex-1" @click.stop="emit('assign', task)">分派</button>
      <button v-if="canComplete" class="btn btn-success btn-sm flex-1" @click.stop="emit('complete', task)">完成</button>
    </div>
  </div>
</template>
