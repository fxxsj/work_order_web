  ...
  <BaseDialog :show="dialogVisible" title="更新任务" width="normal" @close="handleClose; dialogVisible = false;">
    <div class="space-y-4">
      ...
      <div v-if="task" class="flex items-start gap-3">
        ...
        <InputNumber :model-value="task.quantity_completed || 0" disabled class="flex-1" />
      </div>
      <div class="flex items-start gap-3">
        <label class="w-28 text-sm text-gray-600 dark:text-gray-400 pt-2">本次完成数量</label>
        <div class="flex-1">
          <InputNumber v-model="formData.quantity_completed" :min="0" :max="task?.production_quantity ? (task.production_quantity - (task.quantity_completed || 0)) : 999999" class="w-full" />
          <div v-if="task?.production_quantity" class="text-xs text-gray-400 mt-1">
            ...
            <span v-if="(task.quantity_completed || 0) + (formData.quantity_completed || 0) >= task.production_quantity" class="text-success-600">（完成数量将达到计划数量）</span>
            <span v-else class="text-warning-600">（完成数量未达到计划数量）</span>
          </div>
        </div>
      </div>
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1">
          <InputNumber v-model="formData.quantity_defective" :min="0" class="w-full" />
          <div class="text-xs text-gray-400 mt-1">当前不良品：{{ task?.quantity_defective || 0 }}，更新后：{{ (task?.quantity_defective || 0) + (formData.quantity_defective || 0) }}</div>
        </div>
      </div>
      <div v-if="isDesignArtworkTask" class="flex items-start gap-3">
        ...
        <div class="flex-1">
          <Select v-model="formData.artwork_ids" :options="artworkOptions" multiple filterable placeholder="请选择图稿" class="w-full" :loading="loadingArtworks" />
      ...
      <div v-if="isDesignDieTask" class="flex items-start gap-3">
        ...
        <div class="flex-1">
          <Select v-model="formData.die_ids" :options="dieOptions" multiple filterable placeholder="请选择刀模" class="w-full" :loading="loadingDies" />
          <div class="text-xs text-gray-400 mt-1">选中的刀模将自动关联到施工单</div>
        </div>
      </div>
      <TextArea v-model="formData.notes" label="任务备注" :rows="3" placeholder="请输入任务备注（可选）" class="w-full" />
    </div>
    <template #footer>
