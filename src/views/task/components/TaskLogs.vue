    ...
    >
      <template #cell-quantity_change="{ row }">
        <span v-if="row.quantity_before !== null && row.quantity_after !== null">{{ row.quantity_before }} → {{ row.quantity_after }}
          <span v-if="row.quantity_increment > 0" class="ml-1 font-bold text-success-600">(+{{ row.quantity_increment }})</span>
          <span v-else-if="row.quantity_increment < 0" class="ml-1 font-bold text-danger-600">({{ row.quantity_increment }})</span>
        </span>
        <span v-else>-</span>
      </template>
