<template>
  <div v-if="task.logs?.length" style="padding: 20px; background-color: #f5f7fa;">
    <div style="font-weight: bold; margin-bottom: 15px; color: #409EFF;">{{ task.work_content }} - 操作记录（{{ task.logs.length }}条）</div>
    <el-table :data="task.logs" border size="small" style="width: 100%;">
      ...
      <el-table-column label="数量变化" width="180">
        <template #default="scope">
          <span v-if="scope.row.quantity_before !== null && scope.row.quantity_after !== null">{{ scope.row.quantity_before }} → {{ scope.row.quantity_after }}
            <span v-if="scope.row.quantity_increment > 0" style="color: #67C23A; margin-left: 5px; font-weight: bold;">(+{{ scope.row.quantity_increment }})</span>
            <span v-else-if="scope.row.quantity_increment < 0" style="color: #F56C6C; margin-left: 5px; font-weight: bold;">({{ scope.row.quantity_increment }})</span>
          </span>
          <span v-else>-</span>
        </template>
      </el-table-column>
  ...
  <div v-else style="padding: 20px; text-align: center; color: #909399;">暂无操作记录</div>
</template>
