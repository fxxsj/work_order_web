<template>
  <BaseDialog :show="dialogVisible" :title="`批量分派 ${taskCount} 个任务`" width="narrow" @close="handleClose; dialogVisible = false;">
    <div class="space-y-4">
      <Select
      ...
      />
      <div class="text-xs text-gray-400 -mt-2">留空则只分派部门，不分派具体操作员</div>
    ...
    <template #footer>
      ...
      <button class="btn btn-primary" :disabled="submitting" @click="handleSubmit">确定分派</button>
    </template>
  </BaseDialog>
</template>
...
const handleDepartmentChange = async (departmentId: any) => {
  ...
  try { operatorList.value = ((await authAPI.getUserList({ department: departmentId })) as any)?.results || [] } catch (error: any) { ErrorHandler.showMessage(error, '加载操作员列表') } finally { loadingOperators.value = false }
}
