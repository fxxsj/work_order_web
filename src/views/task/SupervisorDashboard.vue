<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">主管看板</h1>
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
      <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-md p-6 text-white">
        <h3 class="text-blue-100 text-sm font-medium mb-4">本周新开工单</h3>
        <div class="text-4xl font-bold">{{ overview.newOrders || 0 }}</div>
        <div class="mt-4 text-sm text-blue-100 flex justify-between">
          <span>同比上周</span>
          <span class="flex items-center text-green-300"><Icon name="arrowUp" size="sm" class="mr-1"/>12%</span>
        </div>
      </div>
      
      <div class="bg-gradient-to-br from-amber-500 to-orange-500 rounded-xl shadow-md p-6 text-white">
        <h3 class="text-orange-100 text-sm font-medium mb-4">异常/延期任务</h3>
        <div class="text-4xl font-bold">{{ overview.delayedTasks || 0 }}</div>
      ...
      <div class="bg-gradient-to-br from-emerald-500 to-green-600 rounded-xl shadow-md p-6 text-white">
        <h3 class="text-green-100 text-sm font-medium mb-4">本月完工率</h3>
        <div class="text-4xl font-bold">{{ overview.completionRate || '0%' }}</div>
        <div class="mt-4 text-sm text-green-100 flex justify-between">
          ...
          <div class="w-24 bg-green-800/50 rounded-full h-2 mt-1.5 overflow-hidden">
            <div class="bg-white h-full" :style="{width: overview.completionRate || '0%'}"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-dark-800 rounded-xl shadow-sm border border-gray-100 dark:border-dark-700 overflow-hidden">
      <div class="border-b border-gray-100 dark:border-dark-700 px-6 py-4">
        <h2 class="text-lg font-medium text-gray-900 dark:text-white">各工序负荷监控</h2>
      </div>
      <div class="p-6">
        <div class="space-y-6">
          <div v-for="dept in workload" :key="dept.name" class="flex items-center">
            <div class="w-32 font-medium text-gray-700 dark:text-gray-300">{{ dept.name }}</div>
