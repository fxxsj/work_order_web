<template>
  <div class="die-list">
    <el-card>
      ...
      >
        <el-table-column prop="code" label="刀模编码" width="150"></el-table-column>
        <el-table-column prop="name" label="刀模名称" width="200"></el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="刀模名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入刀模名称"></el-input>
        </el-form-item>
        <el-row :gutter="20">
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="材质">
              <el-input v-model="form.material" placeholder="如：木板、胶板"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
...
<script>
import { dieAPI, productAPI } from '@/api/workorder'
...
export default {
  ...
  computed: {
    ...
    canCreate() {
      return this.hasPermission('workorder.add_die')
    },
    canEdit() {
      return this.hasPermission('workorder.change_die')
    },
    canDelete() {
      return this.hasPermission('workorder.delete_die')
    }
  },
  created() {
    this.loadData()
  ...
  methods: {
    ...
    async handleSubmit() {
      this.$refs.form.validate(async (valid) => {
        ...
        try {
          ...
          if (this.isEdit) {
            await dieAPI.update(this.editId, data)
    ...
    handleDelete(row) {
      this.$confirm(`确定要删除刀模 ${row.name} 吗？`, '提示', {
