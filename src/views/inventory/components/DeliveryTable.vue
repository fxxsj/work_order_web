<template>
  <DataTable :columns="columns" :data="data" :loading="loading" row-key="id">
    <template #cell-order_number="{ row }">{{ row.order_number }}</template>
    <template #cell-customer_name="{ row }">{{ row.customer_name }}</template>
    <template #cell-sales_order_number="{ row }">{{ row.sales_order_number }}</template>
    <template #cell-receiver_name="{ row }">{{ row.receiver_name }}</template>
    <template #cell-receiver_phone="{ row }">{{ row.receiver_phone }}</template>
    <template #cell-delivery_address="{ row }">{{ row.delivery_address }}</template>
    <template #cell-logistics_company="{ row }">{{ row.logistics_company }}</template>
    ...
    <template #cell-delivery_date="{ row }">{{ row.delivery_date }}</template>
    <template #cell-status="{ row }"><StatusTag :status="row.status" category="delivery" :label="row.status_display" /></template>
