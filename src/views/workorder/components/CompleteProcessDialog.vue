<template>
  <BaseDialog :show="dialogVisible" title="完成工序" width="normal" @close="handleClose; dialogVisible = false;">
    <div class="space-y-4">
      ...
      <div>
        ...
        <div v-if="process?.tasks">
          <div class="mb-3 flex flex-wrap items-center gap-4"><span>总任务数：{{ process.tasks.length }}</span><span>已完成：{{ completedCount }}</span><span v-if="incompleteCount > 0" class="font-bold text-warning-600">未完成：{{ incompleteCount }}</span></div>
          <Alert v-if="incompleteCount > 0" type="warning" :closable="false" class="mb-3"><template #title><p>该工序还有 {{ incompleteCount }} 个任务未完成。建议先完成任务，如需强制完成请勾选下方选项。</p></template></Alert>
        </div>
      </div>
      <div>
        <label class="input-label mb-1.5 block">完成数量</label>
        <InputNumber v-model="form.quantity_completed" :min="0" class="w-full" />
      </div>
      <div>
        <label class="input-label mb-1.5 block">不良品数量</label>
        <InputNumber v-model="form.quantity_defective" :min="0" class="w-full" />
      </div>
      <div v-if="incompleteCount > 0"><Checkbox v-model="form.force_complete" label="强制完成（即使任务未完成）" /></div>
    </div>
    <template #footer>
