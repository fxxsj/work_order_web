  ...
  <div>
    <div v-if="canApprove && workOrder?.approval_status === 'pending'" class="card mt-6">
      ...
      <div class="space-y-4">
        ...
        <div class="flex flex-wrap items-center gap-3">
          <button class="btn btn-success" :disabled="loading" @click="emit('approve', 'approved')"><Icon name="check" size="sm" /> 通过审核</button>
          <button class="btn btn-danger" :disabled="loading" @click="emit('approve', 'rejected')"><Icon name="x" size="sm" /> 拒绝审核</button>
        </div>
      </div>
    </div>
    <div v-if="canResubmit && workOrder?.approval_status === 'rejected'" class="card mt-6">
      ...
      <div class="space-y-4">
        ...
        <button class="btn btn-primary" :disabled="loading" @click="emit('resubmit')"><Icon name="upload" size="sm" /> 提交审核</button>
      </div>
    </div>
  </div>
</template>
