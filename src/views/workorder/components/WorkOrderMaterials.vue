<template>
  <div class="mt-6">
    ...
    <SummaryTable v-if="materials?.length" :columns="columns" :data="materials" :row-key="rowKey">
      <template #cell-material_name="{ row }">
        {{ row.material_name }} ({{ row.material_code }})
      </template>
      <template #cell-purchase_status="{ row }">
        <StatusTag :status="row.purchase_status" :label="row.purchase_status_display" category="materialPurchase" size="small" />
      </template>
      <template #cell-actions="{ row }">
        <button class="btn btn-ghost btn-sm text-primary-600" @click="emit('update-material', row)">编辑</button>
      </template>
    </SummaryTable>
