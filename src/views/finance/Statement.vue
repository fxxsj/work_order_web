  ...
  <div class="statement-container">
    ...
    <div class="table-section">
      ...
      >
        ...
        <el-table-column prop="debit_amount" label="借方发生额" width="120" align="right">
          <template #default="{ row }">
            ¥{{ row.debit_amount ? row.debit_amount.toLocaleString() : '-' }}
          </template>
        </el-table-column>
        <el-table-column prop="credit_amount" label="贷方发生额" width="120" align="right">
          <template #default="{ row }">
      ...
      <div v-if="currentStatement">
        ...
        <el-descriptions :column="3" border>
          ...
          <el-descriptions-item label="状态">
            <el-tag :type="getStatusType(currentStatement.status)">{{ currentStatement.status_display }}</el-tag>
            ...
            <el-col :span="6">
              <el-card>
                <div class="balance-item">
                  <div class="balance-label">期初余额</div>
                  <div class="balance-value">¥{{ currentStatement.opening_balance ? currentStatement.opening_balance.toLocaleString() : '-' }}</div>
                </div>
              </el-card>
            </el-col>
            <el-col :span="6">
              <el-card>
                <div class="balance-item">
                  <div class="balance-label">借方发生额</div>
                  <div class="balance-value debit">¥{{ currentStatement.debit_amount ? currentStatement.debit_amount.toLocaleString() : '-' }}</div>
                </div>
              </el-card>
            </el-col>
            <el-col :span="6">
              <el-card>
                <div class="balance-item">
                  <div class="balance-label">贷方发生额</div>
                  <div class="balance-value credit">¥{{ currentStatement.credit_amount ? currentStatement.credit_amount.toLocaleString() : '-' }}</div>
                </div>
              </el-card>
            </el-col>
            <el-col :span="6">
              <el-card>
                <div class="balance-item">
                  ...
                  <div class="balance-value closing">¥{{ currentStatement.closing_balance ? currentStatement.closing_balance.toLocaleString() : '-' }}</div>
                </div>
              </el-card>
            </el-col>
          </el-row>
        ...
        <!-- 对账明细 -->
        <div v-if="currentStatement.items && currentStatement.items.length > 0" class="statement-items">
          <h3>对账明细</h3>
          <el-table :data="currentStatement.items" border style="width: 100%; margin-top: 10px;" max-height="300">
            <el-table-column prop="date" label="日期" width="120" />
            <el-table-column prop="document_type_display" label="单据类型" width="120" />
            <el-table-column prop="document_number" label="单据号" width="150" />
        ...
        <!-- 确认信息 -->
        <div v-if="currentStatement.status === 'confirmed'" class="confirm-info">
          <h3>确认信息</h3>
          <el-descriptions :column="2" border style="margin-top: 10px;">
            <el-descriptions-item label="确认人">{{ currentStatement.confirmed_by_name || '-' }}</el-descriptions-item>
            <el-descriptions-item label="确认时间">{{ currentStatement.confirmed_at || '-' }}</el-descriptions-item>
            <el-descriptions-item label="确认备注" :span="2">{{ currentStatement.confirm_notes || '-' }}</el-descriptions-item>
          </el-descriptions>
        </div>
      </div>
  ...
  methods: {
    ...
    async fetchStatementList() {
      ...
      try {
        ...
        if (this.filters.statement_type) {
          params.statement_type = this.filters.statement_type
        }
    ...
    async fetchPartners() {
      try {
        ...
        console.error('获取对方单位列表失败', error)
      }
    },
    ...
    async handleView(row) {
      try {
        const response = await getStatementDetail(row.id)
        this.currentStatement = response
        this.detailDialogVisible = true
      } catch (error) {
        this.$message.error('获取对账单详情失败')
        console.error(error)
      }
    },
    ...
    async handleGenerate() {
      this.$refs.statementFormRef.validate(async (valid) => {
        ...
        try {
          const data = {
            statement_type: this.statementForm.statement_type,
            partner: this.statementForm.partner,
            period_start: this.statementForm.period[0],
            period_end: this.statementForm.period[1],
            statement_date: this.statementForm.statement_date,
            notes: this.statementForm.notes
          }
